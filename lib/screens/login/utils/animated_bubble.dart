...
class _AnimatedBubbleBackgroundState extends State<AnimatedBubbleBackground>
  ...
  void initState() {
    ...
    _bubbles = List.generate(_numBubbles, (index) {
      ...
      return _Bubble(
        ...
        opacity:
            _random.nextDouble() * 0.05 + 0.02, // Opacidade entre 0.02 e 0.07
        // Cor sutil, pode variar se desejar
        color: Colors.white.withValues(
          alpha: _random.nextDouble() * 0.03 + 0.01,
        ),
      );
    });
  }
  ...
  Widget build(BuildContext context) {
    return LayoutBuilder(
      // Usar LayoutBuilder para obter as dimensões da tela
            ...
            return AnimatedBuilder(
              ...
              builder: (context, child) {
                ...
                final currentY = lerpDouble(
                  bubble.startY *
                      constraints.maxHeight, // Posição inicial em pixels
                  bubble.endY *
                      constraints.maxHeight, // Posição final em pixels
                ...
                final currentOpacity = lerpDouble(
                  bubble.opacity * 0.5,
                  bubble.opacity,
                  sin(bubble.controller.value * pi),
                );

                return Positioned(
                  left:
                      bubble.startX * constraints.maxWidth - (bubble.size / 2),
